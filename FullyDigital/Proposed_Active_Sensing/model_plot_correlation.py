...
def plot_array_response_sensing(Wa_all, Wb_all, U, V, idx=1):
    for ii in range(iter_test):
        if ii == 0:
            ...
            Wb = Wb_all[ii][idx]
        response_a = (
            torch.abs(Wa.mH @ V[idx].to(device))).cpu().detach().numpy()
        response_b = (
            torch.abs(Wb.mH @ U[idx].to(device))).cpu().detach().numpy()
        response = response_a
        ...
        for jj in range(Ns):
            (markerline, stemlines, baseline) = plt.stem(
                np.arange(1, 1 + len(response[jj])), response[jj],
                linefmt=linefmt[jj], markerfmt=markerfmt[jj], basefmt='black',
                label='Beamforming vector %d' % (jj + 1))
        ...
        plt.grid()
        plt.savefig(
            './fig/sensing_ns%d_SNR_%d_iter_%d' % (Ns, SNR, ii) + '.pdf',
            bbox_inches='tight')
    plt.show()
...
def plot_array_response_data(Wa_all, Wb_all, U, V, idx=1):
    ...
    plt.grid()
    plt.savefig('./fig/data_ns%d_SNR_%d' % (Ns, SNR) + '.pdf',
                bbox_inches='tight')
    plt.show()


'System parameter'
Na, Nb, Ns, num_iter_train, SNR = 64, 64, 4, 16, 0
sigma2 = 10**(-SNR/10)
...
'Create model'
hidden_size = 512
model = ActiveSensingFramework(hidden_size, iter_test, Ns, Na, Nb).to(device)
...
with torch.no_grad():
    ...
    plot_array_response_data(Wa_final, Wb_final, U, V, idx=6)
