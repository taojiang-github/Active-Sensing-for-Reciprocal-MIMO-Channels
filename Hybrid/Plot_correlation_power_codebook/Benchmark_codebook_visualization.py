...
def plot_array_response_sensing(Wa_all, Wb_all, U, V, idx=2):
    for ii in range(num_iter):
        ...
        plt.xlabel('The eigenvector index')
        plt.ylabel('Inner product value')
        plt.title('The %dth iteration'%(ii+1))
        plt.ylim([0, 1])
        plt.legend(loc=1)
        plt.grid()
        plt.savefig(dir_path+'/array_response/sensing_ns%d_SNR_%d_iter_%d'%(Ns,SNR,ii)+'.pdf',bbox_inches='tight')
    plt.show()
...
plot_array_response_sensing(Wa_all, Wb_all, U, V)